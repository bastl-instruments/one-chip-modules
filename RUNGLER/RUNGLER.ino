...
bool render;
bool cycle;
bool usePin[4]={true,false,false,true};
void loop() { 
  //bitRead(PINB,PINB1);
...
ISR(TIMER1_COMPA_vect)  //audiorate interrupt
{
  ...
  lfoValue++;
 if( lastAnalogValues[0]<900 && analogValues[0]>=900) lfoValue=0;
  bitWrite(PORTB,PB4,lfoFlop);// lfoValue>>7);//(lfoValue>>6)%2);
/*
  bool newValue= 
...
*/
  if(lfoValue==0){
   ...
   // if(lfoFlop) { //rungler
      ...
      runglerOut=map(runglerOut,0,7,0,255);
