...
void loop() { 
  if(isConversionFinished()){
   ...
   // analogChannelRead++;
     ...
     */
    // analogChannelRead=
   // if(analogChannelRead>0)
    analogChannelRead=0;
    connectChannel(analogChannelRead);
    startConversion();
  }

 if(analogValues[0]>900) negate=false;
   else negate=true;
...
{
  bool inA=bitRead(PINB,PINB3);
  bool inB=bitRead(PINB,PINB4);
  bool _or=inA | inB;
  bool _and=inA & inB;
  bool _xor=inA ^ inB;
  if(negate){
  ...
  bitWrite(PORTB,PB0,_xor);
  bitWrite(PORTB,PB1,_and);
  bitWrite(PORTB,PB2,_or);
}
