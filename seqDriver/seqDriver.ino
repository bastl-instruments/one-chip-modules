...
void loop() { 
  if(isConversionFinished()){
     ...
     */
    // analogChannelRead=
    if(analogChannelRead>2) analogChannelRead=0;
    connectChannel(analogChannelRead);
    startConversion();
  }

  //  newState=bitRead(PINB,PINB4);
  address=analogValues[2]>>7;
  bool newState=analogValues[0]>900;
  if(!rstState &&  newState) counter=0;
  rstState=newState;
  _delay_us(250);
...
{
  bool _newState=bitRead(PINB,PINB3);
  if(!clkState &&  _newState) counter++;
  counter;//+=address; RAD
  clkState=_newState;
  bitWrite(PORTB,PB0,bitRead(counter+address,0));
  bitWrite(PORTB,PB1,bitRead(counter+address,1));
  bitWrite(PORTB,PB2,bitRead(counter+address,2));
}
