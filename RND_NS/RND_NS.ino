...
void loop() { 
  ...
  if( lastAnalogValues[2]!= analogValues[2]) setSlew(analogValues[2]);


  _delay_us(250);
  uint32_t sum=0;
  for(uint8_t i=0;i<slewRate;i++){
    uint8_t _index=i+slewIndex;
    sum+=slewBuffer[_index];
  }
  slewOut=sum/slewRate;
...
void randomize(){
  if(freq<900) OCR0A= rand(255);
  else  OCR0A= random(255);
  bitWrite(PORTB,PB2,OCR0A%2);

  slewIndex++;
  slewBuffer[slewIndex]=255-OCR0A;//rand(255);//255-OCR0A;//rand(255);
  //
  OCR0B= slewOut;
}
